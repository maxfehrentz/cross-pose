...
from src.utils.transform_utils import reverse_cam_convention_changes, SWAP_AND_FLIP_WORLD_AXES, SCALE
...
def render_mesh_deformations(viewpoint_camera, mesh, deformed_mesh, registration, control_vertices, control_def):
...
def render_mesh(viewpoint_camera, mesh, registration, wireframe=False):
    plotter = pv.Plotter(off_screen=True, window_size=[viewpoint_camera.image_width, viewpoint_camera.image_height])
    plotter.set_background('black')
...
def render(viewpoint_camera, pc : GaussianModel, bg_color : torch.Tensor, mesh = None, registration = None, scaling_modifier = 1.0, override_color = None, deform=True, render_deformation=False, deformed_mesh=None):
    ...
    if mesh is not None and registration is not None and deformed_mesh is not None:
        ...
        init_wireframe, _ = render_mesh(viewpoint_camera, mesh, registration, wireframe=True)
        deformed_wireframe, _ = render_mesh(viewpoint_camera, deformed_mesh, torch.eye(4), wireframe=True)
